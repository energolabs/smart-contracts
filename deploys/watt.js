...
const REGION = 'CHINA';
var param = web3.eth.abi.encodeParameters(
...
var bytecode = output.contracts['Watt.sol:Watt'].bytecode;
console.log(bytecode);

var contractInfo = {
	abi: JSON.parse(output.contracts['Watt.sol:Watt'].interface)
};
...
async function test_name(foo) {
	...
	console.log(res.outputs[0].toString() == 'WATT');
}
...
async function test_increase(rpc, foo) {
	...
	var rp = await foo.send('increaseSupply', [BALANCE, hexAddress], sender);
	var res = await rp.confirm(1);
	console.log(res.logs[0]._value);
	var res = await foo.call('balanceOf', [hexAddress], sender);
	assert.equal(res.outputs[0].valueOf(), BALANCE);

	var rp = await foo.send('decreaseSupply', [BALANCE, hexAddress], sender);
	var res = await rp.confirm(1);
	console.log(res.logs[0]._value);
	var res = await foo.call('balanceOf', [hexAddress], sender);
	assert.equal(res.outputs[0].valueOf(), 0);

	console.log('balance of %s is %d', hexAddress, res.outputs[0].valueOf());

	var receipt = await foo.send(
		'approve',
		[exchangeAddress, '0x99999999999999999'],
		sender
	);
	...
	console.log(
		...
		hexAddress,
		exchangeAddress,
		res.outputs[0].valueOf()
	);
}

async function go() {
	console.log('create contract for watt');
	var res = await rpc.rawCall('createcontract', [
		bytecode + param.slice(2),
		2500000,
		0.0000004,
		senderAddress
	]);
